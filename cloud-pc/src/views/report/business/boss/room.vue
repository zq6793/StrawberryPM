<template>
  <section class="all-box">
    ...
    <el-table :data="billFlows" @selection-change="selsChange" style="width: 100%;" v-loading="listLoading" border>
      ...
      <el-table-column prop="status" label="出租状态" align="center">
      </el-table-column>
    </el-table>
...
import { operationRoomExcel, chartOperationRoom } from '@/api/api'
export default {
  components: {
    CityStoreSelect,
    DateIntervalSelect,
  ...
  methods: {
    ...
    getBillflow() {
      ...
      chartOperationRoom(para).then(res => {
    ...
    handleExports () {
      ...
      that.$confirm('确认导出选中记录吗？', '提示', {
        ...
        operationRoomExcel(para).then((res) => {
