<template>
  <el-form :inline="true" class="allflow">
...
<script>
import { getCityList, getStoreList, showallTaskflow } from '../../api/api'
export default {
  data () {
    return {
      storesList: [],
  ...
  methods: {
    ...
    modifyFree (row) {
      ...
      if (row.type == '正常退房' || row.type == '违约退房' || row.type == '特殊退房' || row.type == '退房-放弃追缴审批' || row.type == '换房' || row.type == '退房-额外退款') {
        this.$router.push({
          path: '/checkoutflow',
          query: {
            id: row.id,
