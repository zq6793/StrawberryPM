...
Page({
    data: {
        ...
        countryCodes     : [{id:"NORMAL_REFUND",countryName:"正常退房"},
                            ...
                            {id:"NO_LIABILITY",countryName:"特殊退房"}],
        countryCodeIndex : 0,
        checkoutData     : time,
        endContract      : time, //合同结束时间
        checkoutReason   : ["工作调动","室友不和","结束单身","买房","对火花不满","火花清退","不接受涨租","其他原因"],
        reasonIndex      : 0,
        showTScheck      : false,
        isShowByDay      : false, //是否显示按天计算
        isByDay          : false, //是否按天计算
    ...
    bindByDayChange(){
        ...
        that.setData({
            isByDay : !that.data.isByDay
        }) 

        if(that.data.isByDay){
            that.data.object.under_calc_type = "DAY";
        }else{
            that.data.object.under_calc_type = "MONTH";
        }
    },
    ...
    bindCountryCodeChange(e){
        ...
        that.setData({
            ...
            isShowByDay  : show_by_day
        })
        that.data.object.type = that.data.countryCodes[that.data.countryCodeIndex].id;
        that.data.object.refund_time_e = that.data.endContract;
        that.data.object.reason_e = that.data.checkoutReason[that.data.reasonIndex];
    },
            ...
            success: function (res) {
                ...
                request.uploadFile(tempArray[index]).then(respone=>{
                    if(JSON.parse(respone).rescode=== 0){
                        ...
                        that.data.object.check_images = JSON.stringify(tempArray);
                    }else{
                        request.showToast('上传失败!');
                    }
                }).catch(e =>{
    ...
    deteleImage(e){
        ...
        request.deteleImage(images,index,res=>{
            ...
            that.data.object.check_images = JSON.stringify(res);
        });
    },
    ...
    next(e){
        ...
        if(content.coldwater_reading){
            that.utility.push({ type : "COLD_WATER_METER", reading : content.coldwater_reading , time : time , image : that.coldwater_img, weight : weights.coldwater_reading });
        }
        if(content.hotwater_reading){
            that.utility.push({ type : "HOT_WATER_METER", reading : content.hotwater_reading , time : time , image : that.hotwater_img, weight : weights.hotwater_reading });
        }
        if(content.electric_reading){
            that.utility.push({ type : "ELECTRIC_METER", reading : content.electric_reading , time : time , image : that.electric_img, weight : weights.electric_reading });
        }
        if(content.gas_reading){
            that.utility.push({ type : "GAS_METER", reading : content.gas_reading , time : time , image : that.gas_img, weight : weights.gas_reading });
        }

        that.object.utility = JSON.stringify(that.utility);
        ...
        wx.setStorageSync('checkout', that.object);

        if(that.checkout_id){
            wx.navigateTo({
                url : "/pages/checkOut/checkoutBalance?checkout_id="+that.checkout_id
            });
        }else{
            wx.navigateTo({
                url : "/pages/checkOut/checkoutBalance"
            });
        }
    },
