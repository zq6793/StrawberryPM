...
const commonJs    = require('../../common/commonJs.js');
...
Page({
    data: {
        ...
        store_id        : "", //门店ID
        userInfo        : {}, //住户信息
        changeRoomType  : [{'id' : 'SAME', 'multiName': '同门店换房' }], //换房类型
        // changeRoomType  : [{'id' : 'SAME', 'multiName': '同门店换房' },{'id': 'DIFFERENT','multiName': '不同门店换房'}], //换房类型                    
        rommIndex       : 0,
        changeRoomData  : time, //换房日期
        reason          : "",
        messageContent  : {}, //设备读数
        weightsContent  : {}, //设备权重
        readingList     : [], //读数列表
    ...
    onLoad (options) {
        ...
        this.data.object.change_room_type =  that.data.changeRoomType[that.data.rommIndex].id;
    },
    onShow(){  
    },
    /*房间号*/
    ...
    room(){
        ...
        requestV4.getResidentInfobyRoom(para).then(res=>{
            ...
            that.data.object.old_room_id = res.room.ID;
            that.data.object.old_resident_id = res.resident.ID;
        }).catch(e=>{
            console.log(e);
        });
    },
    ...
    /*换房类型*/
    changeRoomTypeChange(e){
        this.setData({
            rommIndex : e.detail.value
        })
        this.data.object.change_room_type =  that.data.changeRoomType[that.data.rommIndex].id;
    },

    /*换房日期*/
    changeRoomDateChange(e){
        this.setData({
            changeRoomData : e.detail.value
        })
        this.data.object.change_time_e = e.detail.value;
    },
    ...
    reason(e){
        ...
        this.data.object.change_remark = e.detail.value;
    },
            ...
            success: function (res) {
                ...
                request.uploadFile(tempArray[index]).then(respone=>{
                    if(JSON.parse(respone).rescode=== 0){
                        ...
                        that.data.object.old_check_images = tempArray;
                    }else{
                        request.showToast('上传失败！');
                    }
                });
    ...
    deteleImage(e){
        ...
        request.deteleImage(images,index,res=>{
            ...
            that.data.object.old_check_images = JSON.stringify(res);
        });
    },
    ...
    nextStep(){
        ...
        // content,weights,reading_list,reading_image,arr
        commonJs.readingInfo(content,weights,reading_list,reading_image,that.utility,res=>{
            console.log(res)
        })
        ...
        switch(that.userInfo.resident.pay_frequency){
            case 1:
            that.object.pay_frequency = '月付'; 
            break;
            case 3:
            that.object.pay_frequency = '季付';
            break;
            case 6:
            that.object.pay_frequency = '半年付';
            break;
            case 12:
            that.object.pay_frequency = '年付';
            break;
        }
        that.object.pay_frequency2 = that.userInfo.resident.pay_frequency;

        that.object.old_meters = that.utility;
        wx.setStorageSync('old_room', that.object);
